...
export function buyOrAuction(state, pid, { idx, price }) {
  return new Promise((resolve) => {
    ...
    const el = open(`
      <h2><span style="color:var(--pc,inherit)">${p.token}</span> ${p.name}, ¿lo compras o se subasta?</h2>
      ${deedHTML(state, idx)}
  ...
  return new Promise((resolve) => {
    ...
    const el = open(`
      ...
      <div class="modal-actions">
        ...
        <button class="btn" data-x="card" ${cards ? '' : 'disabled'}>Usar tarjeta (${cards})</button>
...
export function managePanel(state, pid, { debt } = {}) {
  return new Promise((resolve) => {
    ...
    const rows = mine.map(i => {
      ...
      const btns = [];
      if (canBuild(state, pid, i)) btns.push(`<button class="btn small primary" data-a="build" data-i="${i}">+ Casa $${s.houseCost}</button>`);
      if (canSellBuilding(state, pid, i)) btns.push(`<button class="btn small" data-a="sellBuilding" data-i="${i}">Vender casa +$${s.houseCost / 2}</button>`);
    ...
    const el = open(`
      ...
      ${state.bank.houses === 0 ? '<p class="hint">¡No hay cemento! El banco se quedó sin casas.</p>' : ''}`);

    el.querySelectorAll('[data-a]').forEach(b => {
      b.onclick = () => {
        const a = b.dataset.a;
        if (a === 'bankrupt') {
          // Two-step confirm (sandbox-safe: native confirm() can be blocked in iframes).
          if (!b.dataset.armed) {
            b.dataset.armed = '1';
            b.textContent = '¿SEGURO? Toca otra vez pa’ quebrar';
            return;
          }
          closeModal();
...
export function renderAuction(state, view, human) {
  ...
  const humanControls = human ? `
    ...
    <div class="modal-actions">
      <button class="btn danger" data-x="pass">Me rajo</button>
    ...
    const render = () => {
      ...
      const col = (owner, side) => {
        ...
        return `
          <div class="trade-col">
            ...
            ${owner.jailCards.length ? `<label class="trade-item"><input type="checkbox" data-jail="${side}">Tarjeta de salir de la cárcel (${owner.jailCards.length})</label>` : ''}
          </div>`;
      };
      const el = open(`
      ...
      el.querySelector('[data-x=propose]').onclick = () => {
        ...
        if (!tradeValid(state, trade)) {
          let err = el.querySelector('.inline-error');
          if (!err) {
            err = document.createElement('p');
            err.className = 'inline-error';
            el.appendChild(err);
          }
          err.textContent = 'Ese trade no cuadra: tiene que haber algo en la mesa y los números tienen que dar.';
          return;
        }
        closeModal();
        resolve(trade);
...
export function tradeRespond(state, pid, trade) {
  return new Promise((resolve) => {
    ...
    const list = (side) => {
      ...
      if (side.jailCards) bits.push(`${side.jailCards} tarjeta(s) de la cárcel`);
...
export function gameOver(state, personaOf) {
  return new Promise((resolve) => {
    ...
    const el = open(`
      <div class="winner-splash">FIN DEL JUEGO</div>
